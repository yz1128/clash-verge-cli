#!/usr/bin/env python3
"""
Clash Verge CLI Harness - Enhanced Edition
A comprehensive command-line interface for Clash Verge VPN client
"""
...
# Config directory
CONFIG_DIR = Path.home() / "Library/Application Support/io.github.clash-verge-rev.clash-verge-rev"
...
class ClashVergeCLI:
    ...
    def test_proxy_latency(self, proxy_url: str, test_url: str = None) -> Optional[int]:
        """Test proxy latency in milliseconds"""
        if test_url is None:
...
def activate(ctx, profile_name):
    ...
    target = None
    for p in profiles:
        if (p.get('name') == profile_name or 
            p.get('file', '').replace('.yaml', '').replace('.js', '') == profile_name or
            p.get('uid') == profile_name):
            target = p
            break
...
def proxies(ctx, as_json, group):
    ...
    if as_json:
        result = {
            ...
            'groups': [{'name': g.get('name'), 'type': g.get('type'), 'proxies': g.get('proxies', [])} for g in groups]
        }
        click.echo(json.dumps(result, ensure_ascii=False, indent=2))
        return
...
def select(ctx, group_name, proxy_name):
    ...
    for g in groups:
        if g.get('name') == group_name:
            # Remove old selection and add new
...
@click.option('--json', 'as_json', is_flag=True, help='Output as JSON')
@click.pass_context
def webui(ctx, as_json):
    """Show Web UI list"""
    client = ctx.obj['client']
    webui_list = client.get_web_ui_list()
    
    if as_json:
        click.echo(json.dumps(webui_list, ensure_ascii=False, indent=2))
        return
    
    click.echo("=== Web UI ===")
    if not webui_list:
        click.echo("No custom Web UI configured")
    for ui in webui_list:
        click.echo(f"• {ui.get('name')}: {ui.get('url')}")
...
def restart(ctx):
    ...
    app_path = "/Applications/Clash Verge.app/Contents/MacOS/clash-verge"
    if os.path.exists(app_path):
        subprocess.Popen([app_path])
        click.echo("✅ Clash Verge restarted")
    else:
        click.echo("❌ Clash Verge app not found")
...
def open_dir(ctx):
    ...
    import webbrowser
    webbrowser.open(str(ctx.obj['client'].config_dir))
...
def open_logs(ctx):
    """Open logs directory"""
    import webbrowser
    webbrowser.open(str(ctx.obj['client'].config_dir / "logs"))
...
def core(ctx):
    ...
    click.echo(f"Clash Core: {verge.get('clash_core', 'verge-mihomo')}")
...
def refresh(ctx):
    ...
    # Touch profiles.yaml to trigger reload
    profiles_yaml = client.config_dir / "profiles.yaml"
...
@click.option('--interval', default=120, help='Check interval in seconds')
@click.option('--max-latency', default=3000, help='Max latency in ms')
def guardian(once, interval, max_latency):
    """Run MiniMax Guardian (auto-monitor and heal)"""
    import sys
    sys.path.insert(0, os.path.dirname(__file__))
    
    # Import the guardian module
    import importlib.util
    spec = importlib.util.spec_from_file_location("minimax_guardian", 
        os.path.join(os.path.dirname(__file__), "minimax_guardian.py"))
    module = importlib.util.module_from_spec(spec)
    
    click.echo("🛡️  Starting MiniMax Guardian...")
    click.echo(f"   Max latency: {max_latency}ms, Interval: {interval}s")
    click.echo("   Press Ctrl+C to stop\n")
    
    # Run directly
    os.environ['PYTHONPATH'] = os.path.dirname(__file__)
    guardian_script = os.path.join(os.path.dirname(__file__), "minimax_guardian.py")
    if once:
        os.system(f'python3 "{guardian_script}" --once')
    else:
        os.system(f'python3 "{guardian_script}"')
...
def health():
    ...
    try:
        ...
        req.add_header('Authorization', 'lingion')
    ...
    try:
        import requests
        start = time.time()
        r = requests.get('https://api.minimax.io/anthropic', 
                        proxies={'http': 'socks5://127.0.0.1:7897',
